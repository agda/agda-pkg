...
def installFromLocal(libname, src, version, no_defaults, cache):
  ...
  # assert version != "", "--version no available for local"
...
def install(ctx, libnames, src, version, no_defaults, cache, url, git, branch):
  ...
  for libname in libnames:
    ...
    if git and isGit(libname):
      return installFromGit(libname, src, version, no_defaults, cache, branch)
    
    if url and isURL(libname):
      return installFromURL(libname, src, version, no_defaults, cache)
    
    if isLocal(libname):
      return installFromLocal(libname, src, version, no_defaults, cache)
    
    if isIndexed(libname):
      return installFromIndex(libname, src, version, no_defaults, cache)
